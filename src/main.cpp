...
int main() {
    ...
    // NOTE(Lag): My fight mode pin = gp7(My)
    std::array<uint8_t, 5> modePins = { 6, 5, 4, keyboardPin, 7 }; // DO NOT USE PIN GP15
    ...
    // 10 - GP7 - My: Fighting game pad
    if (!gpio_get(7)) USBConfigurations::WiredFightPadPro::enterMode([](){
        DACAlgorithms::WiredFightPadProFG::actuateWFPPFGReport(GpioToButtonSets::F1::defaultConversion());
    });
